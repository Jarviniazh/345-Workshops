...
namespace seneca
{
	Filesystem::Filesystem(const std::string& filename, const std::string& root)
	{
		...
		try
		{
			...
			// If any exception is caught, clean up and rethrow
			delete m_root;  // This will clean up the entire directory tree if properly implemented in Directory's destructor
			m_root = nullptr;  // Prevent dangling pointer
			m_current = nullptr;
			throw;  // Rethrow the current exception to maintain exception propagation
		}
